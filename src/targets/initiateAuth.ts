...
const verifyPasswordChallenge = (user: User, body: Input, userPool: UserPoolClient): PasswordVerifierOutput => ({
  ChallengeName: 'PASSWORD_VERIFIER',
  ChallengeParameters: {},
...
export const InitiateAuth = ({ codeDelivery, cognitoClient, triggers }: Services): InitiateAuthTarget => async (
  ...
  if (
    (userPool.config.MfaConfiguration === 'OPTIONAL' && (user.MFAOptions ?? []).length > 0) ||
