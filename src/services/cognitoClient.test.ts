...
describe('Cognito Client', () => {
  ...
  it('creates a database for clients', async () => {
    ...
    const createDataStore = (jest.fn(() => mockDataStore) as unknown) as CreateDataStore;
    await createCognitoClient({ Id: 'local', UsernameAttributes: [] }, createDataStore, createUserPoolClient);
  ...
  describe('getUserPoolForClientId', () => {
    it("throws if client isn't registered", async () => {
      ...
      await expect(cognitoClient.getUserPoolForClientId('testing')).rejects.toBeInstanceOf(ResourceNotFoundError);
