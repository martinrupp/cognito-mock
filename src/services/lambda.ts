...
export type CreateLambda = (config: FunctionConfig, lambdaClient: AWS.Lambda) => Lambda;

export const createLambda: CreateLambda = (config, lambdaClient) => ({
  enabled: (lambda) => !!config[lambda],
  async invoke(trigger: keyof FunctionConfig, event: UserMigrationEvent | PostConfirmationEvent) {
    ...
    log.debug(`Invoking "${functionName}" with event`, JSON.stringify(lambdaEvent, undefined, 2));
    ...
    log.debug(`Lambda completed with StatusCode=${result.StatusCode} and FunctionError=${result.FunctionError}`);
